...
y_{ij} &\sim \textrm{Bern}(\phi_{z_{i \rightarrow j}z_{i \leftarrow j}}),\quad \phi_{kk'} \sim \textrm{Beta}(\lambda_0, \lambda_1), \\& \textrm{  \textit{for unweighted graphs (MMSB)}} \\
y_{ij} &\sim \textrm{Poi}(\phi_{z_{i \rightarrow j}z_{i \leftarrow j}}), \quad \phi_{kk'} \sim \textrm{Gamma}(r, \frac{p}{1-p}), \\& \textrm{  \textit{for weighted graphs (WMMSB)}}
\end{align*}
%
