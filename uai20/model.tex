...
The Poisson-Gamma combination in WMMSB is interesting as it directly leads to: $y_{ij}|_{Z} \sim \textrm{NB}(r,p)$, where $\textrm{NB}$ denotes the negative binomial distribution. This latter distribution allows one to represent overdispersed count data and has been used in different contexts.%, as in computational linguistics to model bursts in texts \cite{church-1995}.
...
%%
%\begin{equation*}
%\log p({\B{Y}} | \Omega) \ge \E_{q}[\log p({\B{Y}}, \Pi\ | \Omega)] + \textrm{H}[q(\Pi)]
%\end{equation*}
%%
...
%    \frac{\E_{q}[p_{kk'}](N^{\B{Y}}_{kk'} + \E_{q}[r_{kk'}])}{N^{{\Bs{\Phi}}}_{\bm{.}kk'} - \E_{q}[p_{kk'}] + 1}  & \textrm{ for WMMSB-bg}
    \end{cases}
\end{align*}

\textbf{Beta-Gamma augmentation} For WMMSB-bg, the derivation is slightly more complex and is fully detailed in Appendix~\ref{app:collap}. We just provide here the main steps of this derivation.
...
%%
%\begin{equation*}
%q(\Pi) = q({\Bs{\Theta}}, {\Bs{\Phi}}|Z, {\B{R}}, {\B{P}})q(Z)q({\B{R}})q({\B{P}})
%\end{equation*}
%%
...
%p_{kk'} \sim \textrm{Beta}(c\epsilon + N^{\B{Y}}_{kk'}, c(1-\epsilon) + N^{\Bs{\Phi}}_{kk'}\E_{q}[r_{kk'}])
%\end{equation}
%%
so that:
\[
...
%\begin{align}
%\hat N^{{\Bs{\Theta}}}_{\rightarrow ik} += \frac{1}{|s_i|} \frac{1}{Cg(s_i)} \sum_{k'} \gamma_{ijkk'}, & \,\, \hat N^{{\Bs{\Theta}}}_{\leftarrow jk'} = \frac{1}{Cg(s_i)} \sum_{k} \gamma_{ijkk'} \nonumber \\
%\hat N^{{\Bs{\Phi}}}_{xkk'} += \frac{1}{|s_i|} \frac{1}{Cg(s_i)} \gamma_{ijkk'}, & \,\, \hat N^{{\B{Y}}}_{kk'} += \frac{1}{|s_i|} \frac{1}{Cg(s_i)} \gamma_{ijkk'} y_{ij} \nonumber
%\end{align}
...
%\begin{align}
%N^{{\Bs{\Theta}}}_{\rightarrow ik} \leftarrow (1 - \rho^{i,{\Bs{\Theta}}}_t) N^{{\Bs{\Theta}}}_{\rightarrow ik} + \rho^{i,{\Bs{\Theta}}}_t \hat N^{{\Bs{\Theta}}}_{\rightarrow ik}, & \,\,\, N^{{\Bs{\Theta}}}_{\leftarrow jk'} \leftarrow (1 - \rho^{i,{\Bs{\Theta}}}_t) N^{{\Bs{\Theta}}}_{\leftarrow jk'} + \rho^{i,{\Bs{\Theta}}}_t \hat N^{{\Bs{\Theta}}}_{\leftarrow jk'} \nonumber \\
%N^{{\Bs{\Phi}}}_{xkk'} \leftarrow (1 - \rho^{{\Bs{\Phi}}}_t) N^{{\Bs{\Phi}}}_{xkk'} + \rho^{{\Bs{\Phi}}}_t \hat N^{{\Bs{\Phi}}}_{xkk'}, & \,\,\, N^{{\B{Y}}}_{kk'} \leftarrow (1 - \rho^{{\B{Y}}}_t) + \rho^{{\B{Y}}}_t \hat N^{{\B{Y}}}_{kk'} \nonumber
%\end{align}
...
Lastly, to be able to efficiently compute such quantities as $N^{{\Bs{\Phi}}^{-ij}}$ used for the computation of the link probability, one needs to store in memory, for each pair of nodes $(i,j)$, a $K \times K$ matrix, which is not feasible for large networks. Thus, following \cite{foulds2013stochastic}, we replace here $N^{{\Bs{\Phi}}^{-ij}}$ by $N^{{\Bs{\Phi}}}$, which amounts to assume that the contribution of each individual pair of nodes is negligible compared to all other pairs, a reasonable assumption when the network is large. %The complete code for the inference will be made available in GitHub upon acceptance.
...
%
%\begin{algorithm}
%\KwIn{Random initialization of $N^{\Bs{\Theta}}, N^\Phi, N^{\B{Y}}$.}
%\KwOut{${\Bs{\Theta}}h, \Phih$.}
%\Begin{
%$t \gets 0$ \\
...
%        \If{burn-in finished}{
%            Compute intermediate gradient $\hat N^{\Bs{\Theta}}$ from \eqref{eq:sss}.\\
%            Update global statistic $N^{{\Bs{\Theta}} (t)}$.\\
%            Update gradient step $\rho^{\Bs{\Theta}}_t$.\\
%        }
%    }
%
%    Compute intermediate gradient $\hat N^\Phi$ and $\hat N^Y$  from \eqref{eq:sss}.\\
%    Update global statistics $N^{\Phi (t)}$ and $N^{Y (t)}$.\\
%    Update gradient steps $\rho^\Phi_t$ and $\rho^{\Bs{\Theta}}_t$.\\
