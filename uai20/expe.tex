...
As one can note, the overall best performing model is WMMSB-bg, which yields the best results on 8 out of 9 datasets, while EPM yields the best performance on moreno\_names even though the difference with respect to WMMSB-bg on fb\_uc is not large. EPM, performs slightly better than the other model except for WMMSB-bg on 6 datasets but to a lesser extent than the results of WMMSB-bg versus the other.
