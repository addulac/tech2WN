...
Mixed-Membership Stochastic Blockmodels (MMSB) extend the Stochastic Blockmodel (SBM) \cite{airoldi2009mixed} by allowing nodes to "belong" to several blocks (or classes) through a Dirichlet probability distribution. In the case of unweighted graphs, the prediction of a new link is based on a Bernoulli distribution that link distribution depending on the blocks interaction only. % => structural equivalence
