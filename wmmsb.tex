...
\begin{abstract}
We propose in this study a new mixed-membership stochastic block model for weighted networks that addresses the main limitation of previously proposed weighted stochastic block models. We furthermore show how one can infer the parameters of this model through a coupling of collapsed and stochastic variational inference, so that the model can be deployed on large networks comprising millions of edges. The experiments, conducted on several diverse real-world networks, show that this model outperforms previously proposed models in different settings, especially when the number of edges used to learn the model is limited.
